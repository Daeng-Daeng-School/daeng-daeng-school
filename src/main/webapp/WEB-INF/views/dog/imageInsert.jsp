...
<script>
            ...
            <form action="${pageContext.servletContext.contextPath}/dog/insert" method="post" enctype="multipart/form-data" id="profileForm${formCounter}" class="profileForm">
                <div class="input_image">
                    <label for="inputImage${formCounter}" class="btn_model">
                        ...
                        <input type="file" id="inputImage${formCounter}" name="inputImage" class="dog_image" accept="image/*" style="display: none;">
                    </label>
                </div>
                <div class="input_box">
    ...
    function addProfileImage(imageSrc) {
        const profileTemplate = `
            <div class="profile">
                <img src="${imageSrc}" alt="반려견 이미지" class="profile-img">
            </div>
        `;
        document.getElementById('profileContainer').insertAdjacentHTML('beforeend', profileTemplate);
    }
    ...
    $(document).on('submit', 'form', function(event) {
        ...
        $.ajax({
            ...
            success: function(response) {
                alert('등록 완료!');
                var imageUrl = response.imageUrl; // 서버에서 반환한 이미지 URL
                addProfileImage(imageUrl);
                form.reset(); // 폼 초기화
            },
            error: function() {
                alert('등록 실패!');
